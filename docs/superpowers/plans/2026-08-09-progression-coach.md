...
1. The counted work sets must agree with each other within `WEIGHT_TOLERANCE_KG` (mixed weights return `INSUFFICIENT_DATA/MANUAL_WEIGHT_DEVIATION` with `IGNORE`). The agreed weight is the evaluation basis: success proposals step up from the actually trained weight, so a plan converges to reality instead of stalling when the user deviates from the plan target.
