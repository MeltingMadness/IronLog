...
class BackupPayloadValidatorTest {
    ...
    @Test
    fun duplicatePlanExercisePlanAndOrderPositions_failValidation() {
        val payload = validPayload().copy(
        ...
        assertFalse(result.isValid)
        assertTrue(result.errors.any { it.contains("duplicate plan exercise plan/order", ignoreCase = true) })
    }

    @Test
    fun duplicateMetaPlanItemMetaPlanAndOrderPositions_failValidation() {
        val payload = validPayload().copy(
            metaTrainingPlans = listOf(
                BackupMetaTrainingPlan(id = 7L, name = "Meta 1", createdAt = 1000L)
            ),
        ...
        assertFalse(result.isValid)
        assertTrue(result.errors.any { it.contains("duplicate meta plan item meta plan/order", ignoreCase = true) })
    }

    @Test
    fun nonPositiveIds_failValidationOnEveryTable() {
